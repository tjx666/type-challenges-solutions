...
// 
type PermutationUnion<T extends string, U extends string = T> = [T] extends [never]
    ? ''
    : T extends string
    ? `${T | ''}${PermutationUnion<Exclude<U, T>>}`
    : never;
type StringToUnion<S extends string> = S extends `${infer F}${infer R}`
    ? F | StringToUnion<R>
    : never;
type AllCombinations<S extends string> = PermutationUnion<StringToUnion<S>>;

// 整体思路：其实就是回溯算法，不断占坑位，每占一个坑位就要减一个分支，需要注意这里空位允许为空，也就是空串
