...
type Split<S extends string, SEP extends string, R extends string[] = []> = string extends S
    ? string[]
    : S extends `${infer Front}${SEP}${infer Back}`
    ? SEP extends ''
        ? [...R, Front, ...Split<Back, SEP, []>]
        : [...R, ...Split<Front, SEP, []>, ...Split<Back, SEP, []>]
    : SEP extends ''
    ? []
    : [S];

type R = Split<'Hi! How are you?', ''>;
