...
// 看一下 599 题
// { a: 1, b: 2 } 不等于 { a: 1 } & { b: 2 }
// 所以 merge 属性不能直接使用交叉类型
type ReadonlyMerge<F, S> = {
    readonly [K in keyof F | keyof S]: K extends keyof S ? S[K] : K extends keyof F ? F[K] : never;
};

type UppercaseFirstLetter<S extends string> = S extends `${infer First}${infer Rest}`
...
> = T extends readonly [infer First, ...infer Rest]
    ? ReadonlyMerge<
          {
              [P in UppercaseFirstLetter<First & string>]: N extends true ? Index['length'] : First;
          },
          Enum<Rest, N, N extends true ? [...Index, unknown] : Index>
      >
    : {};
