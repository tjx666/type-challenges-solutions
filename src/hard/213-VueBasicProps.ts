...
type Computed<T extends Record<string, Function>> = {
    [K in keyof T]: T[K] extends (...args: any[]) => infer R ? R : never;
};

type GetInstanceType<C> = C extends (...args: any) => any
    ? ReturnType<C>
    : C extends new () => infer InstanceType
    ? InstanceType
    : never;

type ConstructorUnionToInstanceTypeUnion<U, E = U> = E extends U ? GetInstanceType<E> : never;

type GetProps<T> = {
    [P in keyof T]: T[P] extends { type: infer Type }
        ? Type extends any[]
            ? ConstructorUnionToInstanceTypeUnion<Type[number]>
...
declare function VueBasicProps<P, D, C extends Record<string, Function>, M>(
    options: {
        props: P;
        data: (this: GetProps<P>) => D;
        computed?: C & ThisType<D & C>;
        methods?: M & ThisType<GetProps<P> & D & Computed<C> & M>;
    } & ThisType<null>,
): any;
